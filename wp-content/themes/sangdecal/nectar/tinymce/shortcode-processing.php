...
function nectar_testimonial_slider($atts, $content = null) { 
    extract(shortcode_atts(array("autorotate"=>'', 'once_icon' => 0, 'swap_name' => 1, 'name_tag' => 'span'), $atts));
	NectarConfig::set('swap_name', $swap_name);
	NectarConfig::set('image_top', $once_icon);
	NectarConfig::set('name_tag', $name_tag);
    $result = '<div class="col span_12 testimonial_slider" data-autorotate="'.$autorotate.'" data-name_tag="'.$sname_tag.'" data-swap_name="'.$swap_name.'" data-once_icon="'.$once_icon.'"><div class="slides">'.do_shortcode($content).'</div></div>';
	return $result;
}
...
function nectar_testimonial($atts, $content = null) { 
    extract(shortcode_atts(array("name" => '', "quote" => '', 'image' => '', 'extra_html' => '', 'extra_class' => ''), $atts));
	...
	if( $swap_name ) {
		return '<blockquote class="'.$cls.'"><'.$name_tag.' class="name">'.$image.$name.'</'.$name_tag.'>'.$quote.$extra_html.'</blockquote>';
	}
    return '<blockquote class="'.$cls.'">' .$quote. '<'.$name_tag.' class="name">'.$image.$name.'</'.$name_tag.'>'.$extra_html.'</blockquote>';
}
...
function nectar_carousel($atts, $content = null) {  
    extract(shortcode_atts(array("carousel_title" => 'Title', "scroll_speed" => 'medium', 'easing' => 'easeInExpo', 'autorotate' => '', 'max_cols' => 4, 'col_width' => 264, 'scroll_num' => '1', 'full_width' => 'false'), $atts));
...
function nectar_slider_processing($atts, $content = null) {
	...
	extract(shortcode_atts(array("arrow_navigation" => 'false', "autorotate"=> '', "slider_transition"=> 'swipe', "flexible_slider_height"=> '', "min_slider_height"=> '', "loop" => 'false', 'fullscreen' => 'false', "bullet_navigation" => 'false', "parallax" => 'false', "full_width" => '', "slider_height" => '650', "desktop_swipe" => 'false', "location" => '', 'max_columns' => 4), $atts));
